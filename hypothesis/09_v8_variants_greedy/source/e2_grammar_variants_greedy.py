"""
e2_grammar_variants_greedy.py
==============================
提案 E-2（修正版）: V8 文法バリアント感度分析 — 貪欲分割アルゴリズム使用

hypothesis/08_v8_sensitivity の e2_grammar_variants.py からの修正点:
  【旧】find_splits_variant: 最短先頭基優先（first-fit）
       → i=1 から試し最初に有効な分割を返す
       → "ck" を 'c' + 'k...' に分解してしまい compound_words.txt と 99.6% 異なる
  【新】parse_word_greedy: parse_v4b.py の parse_base_greedy と等価
       → スロット順に貪欲に最長マッチした基を逐次確定
       → V0_baseline が compound_words.txt と完全一致することを確認済み

文法バリアント:
  V0_baseline  : SLOTS_V8 オリジナル（基準 → compound_words.txt と同一分割になるはず）
...
実行:
  ...
  PYTHONPATH=/home/practi/work_voy/.venv/lib/python3.10/site-packages \\
    python3.10 hypothesis/09_v8_variants_greedy/source/e2_grammar_variants_greedy.py
"""
...
# ── 設定 ──────────────────────────────────────────────────────────────────────
OUT_DIR = ROOT / "hypothesis/09_v8_variants_greedy/results"
OUT_DIR.mkdir(parents=True, exist_ok=True)

K_LIST = [7, 8]

# ── 文法バリアント定義（hypothesis/08 と同一） ────────────────────────────────
_SLOTS_V8_BASE = [
    ["l", "r", "o", "y", "s", "v"],
    ["q", "s", "d", "x", "l", "r", "h", "z"],
    ["o", "y"],
...
# ════════════════════════════════════════════════════════════════════════════
# 1. 貪欲分割アルゴリズム（parse_v4b.py の parse_base_greedy と等価）
# ════════════════════════════════════════════════════════════════════════════
def parse_base_greedy(word: str, start: int, slots: list) -> int:
    """
    parse_v4b.py の parse_base_greedy と完全等価。
    start 位置から貪欲にスロット順マッチを行い、到達した終端位置を返す。
    スロットを次々と試し、いずれにもマッチしない時点で停止。
    """
    pos       = start
    next_slot = 0
    n_slots   = len(slots)

    while next_slot < n_slots and pos < len(word):
        found = False
        for slot_idx in range(next_slot, n_slots):
...
def parse_word_greedy(word: str, slots: list, max_bases: int = 4) -> list | None:
    """
    parse_v4b.py の parse_word と完全等価。
    1 基ずつ貪欲に分割し、最大 max_bases 基まで試みる。
    語全体をカバーできた場合のみ分割リストを返す。失敗時は None。
    """
    n   = len(word)
    pos = 0
    bases: list[str] = []
    ...
    for _ in range(max_bases):
        if pos >= n:
            break
        end = parse_base_greedy(word, pos, slots)
        if end == pos:      # マッチが進まない → 解析失敗
            return None
        bases.append(word[pos:end])
        pos = end
        if pos == n:        # 語全体をカバー → 成功
            return bases

    return None  # max_bases 回でカバーできなかった


def generate_compound_list(all_words: list, slots: list) -> dict:
    """
    全語に対して parse_word_greedy を実行し、2 基以上に分割できた語を複合語とする。
    返り値: {word: (base1, base2, ...)}
    """
    compounds = {}
    for word in all_words:
        result = parse_word_greedy(word, slots)
        if result is not None and len(result) >= 2:
            compounds[word] = tuple(result)
    return compounds
...
def save_compound_list(compounds: dict, out_path: Path, variant_name: str, description: str):
    ...
    for word in sorted(compounds.keys()):
        ...
        lines.append(f"[{len(bases)}基] {word}  ->  {bases_str}")
    out_path.write_text("\n".join(lines), encoding="utf-8")
    common.log(f"    複合語リスト保存: {out_path.name}  ({len(compounds):,} 語)")


# ════════════════════════════════════════════════════════════════════════════
# 2. V0_baseline の検証（compound_words.txt との一致確認）
# ════════════════════════════════════════════════════════════════════════════
def verify_baseline(compounds_v0: dict) -> dict:
    """
    V0_baseline の分割が compound_words.txt と一致するか確認する。
    """
    import re
    orig_path = ROOT / "hypothesis/00_slot_model/data/compound_words.txt"
    pattern   = re.compile(r"\[(\d+)基\]\s+(\S+)\s+->\s+(.+)")
    orig      = {}
    for line in orig_path.read_text("utf-8").splitlines():
        m = pattern.match(line.strip())
        if m:
            orig[m.group(2)] = tuple(b.strip() for b in m.group(3).split("+"))

    only_orig   = set(orig)  - set(compounds_v0)
    only_v0     = set(compounds_v0) - set(orig)
    same_split  = {w for w in orig if w in compounds_v0 and orig[w] == compounds_v0[w]}
    diff_split  = {w for w in orig if w in compounds_v0 and orig[w] != compounds_v0[w]}

    return {
        "orig_count":   len(orig),
        "v0_count":     len(compounds_v0),
        "only_orig":    len(only_orig),
        "only_v0":      len(only_v0),
        "same_split":   len(same_split),
        "diff_split":   len(diff_split),
        "diff_examples": [(w, orig[w], compounds_v0[w]) for w in list(diff_split)[:5]],
    }


# ════════════════════════════════════════════════════════════════════════════
# 3. グループ収集 & 役割分析（hypothesis/08 e2 と同一）
# ════════════════════════════════════════════════════════════════════════════
...
def plot_group_rates(role_result: dict, k: int, variant_name: str, out_path: Path):
    ...
    ax.set_title(
        f"[E-2 修正版 貪欲分割] {variant_name}  k={k}  Focus State S{fs}\n"
...
# 5. Markdown レポート
# ════════════════════════════════════════════════════════════════════════════
def build_report(all_variant_results: dict, verify_result: dict,
                 ref_compound_count: int) -> str:
    ...
    lines = [
        ...
        f"| 項目 | 件数 |",
        f"|------|------|",
        f"| 元 compound_words.txt (V4/V8 文法) | {verify_result['orig_count']:,} |",
        f"| V0_baseline (貪欲アルゴリズム) | {verify_result['v0_count']:,} |",
        f"| 元ファイルのみ | {verify_result['only_orig']:,} |",
        f"| V0_baseline のみ | {verify_result['only_v0']:,} |",
        f"| 同一分割 | **{verify_result['same_split']:,}** |",
        f"| 分割点が異なる | {verify_result['diff_split']:,} |",
        "",
    ]

    if verify_result["diff_examples"]:
        lines += ["分割点が異なる例:", "", "| 語 | 元分割 | V0 分割 |", "|---|---|---|"]
        for w, orig, v0 in verify_result["diff_examples"]:
            lines.append(f"| {w} | {' + '.join(orig)} | {' + '.join(v0)} |")
        lines.append("")
...
if __name__ == "__main__":
    common.log("=== E-2（修正版 貪欲分割）: 文法バリアント感度分析 開始 ===")

    # データロード
    ref_compound_splits = common.load_compound_splits()  # 参照用
    ...
    common.log(f"元 compound_words.txt: {len(ref_compound_splits):,} 語")
    ...
    for vname, slots in VARIANTS.items():
        ...
        # ── 貪欲分割で複合語リスト生成 ──────────────────────────────────────
        common.log("  複合語リスト生成中（貪欲分割）...")
        compounds_raw = generate_compound_list(all_words, slots)
        compounds = {
            w: s for w, s in compounds_raw.items()
            if all(c in char2idx for c in w)
        }
        common.log(f"  複合語数: {len(compounds):,}")

        # V0_baseline の検証
        if vname == "V0_baseline":
            verify_result = verify_baseline(compounds)
            common.log(f"  【検証】 compound_words.txt との一致:")
            common.log(f"    同一分割: {verify_result['same_split']:,} / "
                       f"{verify_result['orig_count']:,}  "
                       f"({verify_result['same_split'] / max(verify_result['orig_count'],1) * 100:.1f}%)")
            common.log(f"    分割点が異なる: {verify_result['diff_split']:,}")
            if verify_result['only_orig']:
                common.log(f"    元ファイルのみ: {verify_result['only_orig']:,}")
            if verify_result['only_v0']:
                common.log(f"    V0 のみ: {verify_result['only_v0']:,}")

        # 複合語リスト保存
        save_compound_list(
            compounds, OUT_DIR / f"compounds_{vname}.txt",
    ...
    # レポート生成
    common.log("\nMarkdown レポート生成中...")
    report_path = OUT_DIR / "variant_report_greedy.md"
    report_path.write_text(
        build_report(all_variant_results, verify_result, len(ref_compound_splits)),
        encoding="utf-8",
    )
